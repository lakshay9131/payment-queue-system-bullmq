...
const token_bucket_1 = require("../queue/token-bucket");
describe('TokenBucket', () => {
    it('allows consumption up to capacity, then blocks', () => {
        const bucket = new token_bucket_1.TokenBucket(3, 1); // 3 capacity, slow refill so it won't interfere
    ...
    it('refills over time', async () => {
        const bucket = new token_bucket_1.TokenBucket(1, 10); // refills fast: 10/sec
        expect(bucket.tryConsume()).toBe(true);
    ...
    it('shrinks capacity and refill rate under adapt(1) and restores on reset', () => {
        const bucket = new token_bucket_1.TokenBucket(10, 10, 0.2);
    ...
    it('reports msUntilAvailable > 0 once exhausted', () => {
        const bucket = new token_bucket_1.TokenBucket(1, 5);
        bucket.tryConsume();
        expect(bucket.msUntilAvailable()).toBeGreaterThan(0);
    });
});
//# sourceMappingURL=token-bucket.spec.js.map