...
const circuit_breaker_1 = require("../payment/circuit-breaker");
describe('CircuitBreaker', () => {
    it('trips OPEN after consecutive failures reach the threshold', () => {
        const breaker = new circuit_breaker_1.CircuitBreaker('test-gw', {
            failureThreshold: 3,
        ...
        breaker.recordFailure();
        expect(breaker.getState()).toBe(circuit_breaker_1.CircuitState.CLOSED);
        breaker.recordFailure(); // 3rd consecutive failure -> trips
        expect(breaker.getState()).toBe(circuit_breaker_1.CircuitState.OPEN);
    ...
    it('transitions OPEN -> HALF_OPEN after openDurationMs, then CLOSED after enough trial successes', async () => {
        ...
        expect(breaker.getState()).toBe(circuit_breaker_1.CircuitState.HALF_OPEN);
        breaker.recordSuccess();
