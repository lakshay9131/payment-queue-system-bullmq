...
jest.mock('bullmq', () => {
    const addMock = jest.fn().mockImplementation((name, data, opts) => {
        return Promise.resolve({ id: opts?.jobId ?? 'auto-id', name, data, opts });
    });
    return {
        Queue: jest.fn().mockImplementation((queueName) => ({
            name: queueName,
            add: addMock,
...
describe('QueueManagerService — ingestion (enqueue)', () => {
    ...
    beforeEach(() => {
        ...
        events = { emit: jest.fn() };
        manager = new queue_manager_service_1.QueueManagerService(events);
    });
    it('throws synchronously if the gateway was never registered — never touches BullMQ', async () => {
        await expect(manager.enqueue(makePayment({ gatewayId: 'unknown-gw' }))).rejects.toThrow(/No queue registered for gateway "unknown-gw"/);
    });
    it('routes to the correct per-gateway, per-priority queue', async () => {
        ...
        const QueueMock = require('bullmq').Queue;
        const stripeHighQueueCall = QueueMock.mock.calls.find((c) => c[0] === 'payments-stripe-high');
    ...
    it('routes delayed payments to the scheduled queue, bypassing gateway/priority selection', async () => {
        ...
        const QueueMock = require('bullmq').Queue;
        const scheduledCall = QueueMock.mock.calls.find((c) => c[0] === 'payments-scheduled');
    ...
    it('registerGateway is idempotent — calling it twice does not create duplicate queues', () => {
        ...
        manager.registerGateway('stripe');
        const QueueMock = require('bullmq').Queue;
        const stripeHighCalls = QueueMock.mock.calls.filter((c) => c[0] === 'payments-stripe-high');
